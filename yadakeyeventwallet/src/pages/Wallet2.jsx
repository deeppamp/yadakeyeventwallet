...
const Wallet2 = () => {
  ...
  // Derive blockchain color from selected blockchain
  const blockchainColor = selectedBlockchain?.color || "blue";
  ...
  const walletManager = useMemo(
    () => selectedBlockchain?.id ? walletManagerFactory(selectedBlockchain.id) : null,
  ...
  const { tokenSymbol, wrappedTokenSymbol } = useMemo(() => {
    if (!selectedBlockchain?.isBridge)
      return { tokenSymbol: selectedBlockchain?.symbol || "YDA", wrappedTokenSymbol: "" };
  ...
  useEffect(() => {
    const checkDeploymentStatus = async (appContext) => {
      if (isDeployed && Object.keys(contractAddresses).length > 0) {
        // Only check owner status for bridge blockchains
        if (selectedBlockchain.isBridge) {
    ...
    if (storedPrivateKey && storedWif && storedParsedData) {
      try {
        ...
        if (parsed.blockchain === selectedBlockchain.id) {
          ...
          // Don't call checkOwnerStatus here to avoid infinite loop
        } else {
          setPrivateKey(null);
          setWif("");
          setParsedData(null);
  ...
  useEffect(() => {
    // For bridge blockchains, require parsedData; for non-bridge, only require privateKey
    const shouldCheck = selectedBlockchain.isBridge 
      ? (privateKey && !isInitialized && !isSubmitting && parsedData)
      : (privateKey && !isInitialized && !isSubmitting);
  ...
  const handleSalviumImport = useCallback(
    async (privateKeyObj, address) => {
      try {
        // Store the private key
        setPrivateKey(privateKeyObj);
        
        // Create parsedData object for Salvium
        const parsedDataObj = {
        ...
        localStorage.setItem(
          `walletPrivateKey_sal`,
          JSON.stringify(privateKeyObj)
        );
        localStorage.setItem(
          `walletParsedData_sal`,
          JSON.stringify(parsedDataObj)
        );
        localStorage.setItem(`walletIsInitialized_sal`, "true");
        
        // Fetch balance
        await walletManager.fetchBalance(appContext);
        
        console.log("Salvium wallet imported successfully");
      } catch (error) {
        console.error("Error handling Salvium import:", error);
        notifications.show({
          title: "Error",
          message: "Failed to import Salvium wallet",
          color: "red",
        });
      }
    },
    [walletManager, setPrivateKey, setParsedData, setIsInitialized, appContext]
  );
  ...
  useEffect(() => {
    if (isScannerOpen || isQRModalOpen) {
      close(); // Always close when modal opens
    }
  }, [isScannerOpen, isQRModalOpen, close]);

  // Close navbar when blockchain changes
  useEffect(() => {
    close(); // Always close when blockchain changes
  }, [selectedBlockchain?.id, close]);
          ...
          {!selectedBlockchain.disabled && (
            <>
              <WalletStateHandler
                ...
                styles={styles}
                onSalviumImport={handleSalviumImport}
              />
              {privateKey &&
                isDeployed &&
                (selectedBlockchain.isBridge ? log.length === parsedData?.rotation : true) && (
                  <>
                    <Card
                      withBorder
                      mt="md"
                    ...
                    {isInitialized && (
                      <HardwareWalletConnect
                        currentBalance={balance?.original || balance?.balance || 0}
                        currentCoin={selectedBlockchain.name}
                        currentAddress={parsedData?.address}
                      />
                    )}
                    ...
                    {isInitialized && (selectedBlockchain.isBridge ? parsedData?.rotation === log.length : true) && (
                      <TransactionForm
